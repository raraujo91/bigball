<script>
    ...
    export let userBet;
    let { bet, match, teams, points, scores } = userBet;
    console.log("ub", userBet);
</script>
...
<div class="m-4">
    <div class="text-slate-800 w-full bg-gray-100 rounded-lg p-6 text-4xl">
        ...
        <div class="flex justify-evenly mt-6">
            <span
                class="ring ring-slate-300 uppercase font-bold text-sm rounded-full bg-slate-200 py-1 px-4"
                >{teams.home.name}
    ...
    <div
        class="mt-8 w-full grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 flex justify-center text-center"
    >
        {#if match.finished}
            <div
                class="bg-slate-100 text-slate-900 border border-slate-100 py-4"
            >
